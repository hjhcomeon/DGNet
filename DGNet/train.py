...
from nets.DGNet_training import get_lr_scheduler, set_optimizer_lr, weights_init
from utils.callbacks import EvalCallback, LossHistory
from utils.dataloader import UnetDataset, unet_dataset_collate
from utils.utils import (download_weights, seed_everything, show_config,
                         worker_init_fn)
from utils.utils_fit import fit_one_epoch
from nets.DGNet import get_seg_model
...
if __name__ == "__main__":
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path  = "model_data/DDRNet23s_imagenet.pth"
    #-----------------------------------------------------#
...
## 下面一行代码进行模型的训练，调用的是net文件夹中的UNet.py文件


    # model = Unet(num_classes=num_classes, pretrained=pretrained, backbone=backbone).train()
    # shapex = [input_shape[0], input_shape[1], 3]
    model = get_seg_model(num_classes)
    if not pretrained:
        weights_init(model)
    if model_path != '':
        #------------------------------------------------------#
